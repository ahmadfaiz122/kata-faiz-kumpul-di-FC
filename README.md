...
- **Google OAuth Login** restricted to `@mhs.unesa.ac.id` email addresses, issuing a Laravel Sanctum token stored in `localStorage`.
...
- **Leaderboard & rating** — reputation is calculated from reviews after a transaction is completed.


## Tech Stack

**Backend** — PHP 8.2+, Laravel 12, Laravel Sanctum (auth token), Laravel Socialite (Google OAuth), SQLite (default; MySQL/MariaDB/PostgreSQL/SQL Server supported), Eloquent ORM.

**Frontend** — Svelte 5 (with Runes), Vite, `svelte-spa-router` (hash routing), Tailwind CSS 4, neo-brutalist design (thick borders + solid shadows).

**Communication** — REST API with Bearer token authentication (Sanctum), CORS configured for the frontend origin.
